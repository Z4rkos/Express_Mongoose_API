...
router.post('/register', async (req, res) => {
    ...
    if (emailExists) return res.status(400).json("Email " + req.body.email + " is already registered.");

    // Check if the username is in the database.
    const usernameExists = await User.findOne({username: req.body.username});
    if (usernameExists) return res.status(400).json("Username " + req.body.username + " is already registered.");
